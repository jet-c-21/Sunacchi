// >>> >>> >>> >>> initialize variables of elements on page >>> >>> >>> >>> >>>
...
var settings = null; // global variable, and will be modified in other functions in runtime

function load_settings_to_form(settings) {
    if (settings) {
...
function maxbot_version_api() {
    ...
    $.get(api_url, function () {
        ...
        .done(function (data) {
            $("#maxbot_version").html(data.version); // update displayed version.
        })
        .fail(function () {
            //alert( "error" );
        })
